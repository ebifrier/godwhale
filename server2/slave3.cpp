...
signed char* boardhead() { return &(g_ptree->posi.asquare[0]); }
int rootNrep() { return g_ptree->nrep; }

class singleCmdC {
 public:
  // input
 int valid, issued, picked,
     haveList; // FIXME? need chkList? - do without it for now 10/30
...
void singleCmdC::clear() { memset(this, 0, sizeof(singleCmdC)); }
...
enum { ACCROOT=0, ACCFIRST=1, ACCLIST=2, ACCWARM=3,
       ACCABORT=4, ACCEXPIRE=5, ACCSLEEP=6, ACCRETRY=7, ACCPUZL=8, ACCEND=9 };
//int64_t accumTimes[ACCEND], accumNodes[ACCEND], startTime;
const char* srchtypestr[ACCEND] = { "Root", "First", "List", "Warm",
                                 "Abort", "Expire", "Sleep", "Retry", "Puzl" };
#include "perfsl.h"
...
static int countFlip(int turn, int i) { return ((i%2) ? Flip(turn) : turn); }
...
static void bringPosTo(int leng, mvC* pv, bool setchks = true) {
 ...
 rewindToRoot();  // 12/13/2010 #36 was if (mch) ret else rwdRt();
 ...
 int i = leng;  // 12/18/2010 #47 nsuc set for [leng+1] was missing
...
void resetBonaSearch(tree_t * restrict ptree) {
 ...
 //root_turn = currentRootPos.turn_to_move;
 //  root_turn is set by ini_game(), updated by [un]make_move_root()
...
enum { PROCE_OK = 0, PROCE_ABORT = 1 };

int proce(int nested) {

  // if nested, no cmd should be pending
 if (nested)
   assert(!pendingCmd.havecmd());

  // if cmd has come, put it to pendigCmd(pc)
 //if (!pendingCmd.havecmd())   - this check is done inside getCmd() method
 pendingCmd.getCmd();
   ...
   switch(pcOpc) {
       ...
       case CMD_OPCODE_FWD:
         ...
         { int i, ply;
           ...
           //      g_ptree->history[0][0][i] /= HISTORY_DIVIDE;
           ...
           for ( ply = 0; ply < PLY_MAX; ply++ )
            ...
            //if (!BNS_COMPAT && USE_ADJUST_FMG && !rwdLatch)
...
#if 0
         if (0 /*MEASURE_TIME*/) {
           int64_t now = worldTimeLl();
           int64_t accumNode = accumNodes[ACCROOT] + accumNodes[ACCFIRST] + 
                               accumNodes[ACCLIST] + accumNodes[ACCRETRY];
           int64_t accumTime = accumTimes[ACCROOT] + accumTimes[ACCFIRST] + 
                               accumTimes[ACCLIST] + accumTimes[ACCRETRY];
           int64_t y = accumNode * 100000;
           int64_t walltime = (now - startTime + 1);
           float r = accumTime * 100;
           r /= (walltime + 1);
           y /= (accumTime / 10000 + 1);
           SLTOut("**** accum node %lld srchtime %lld walltime %lld nps %lld srch time ratio %2.1f%%\n",
                  accumNode, accumTime, walltime, y, r);
           r = float(walltime + 1)/100;
           SLTOut("**** root %2.1f%% first %2.1f%% list %2.1f%% retry %2.1f%% warm %2.1f%%\n",
                  float(accumTimes[ACCROOT])/r, float(accumTimes[ACCFIRST])/r,
                  float(accumTimes[ACCLIST])/r, float(accumTimes[ACCRETRY])/r,
                  float(accumTimes[ACCWARM])/r );
           SLTOut("**** abort %2.1f%% expire %2.1f%% sleep %2.1f%%\n",
                  float(accumTimes[ACCABORT])/r, float(accumTimes[ACCEXPIRE])/r,
                  float(accumTimes[ACCSLEEP])/r );
           forr(k,0,ACCEND-1)
             accumTimes[k] = accumNodes[k] = 0;
         }
#endif
       ...
       case CMD_OPCODE_NOTIFY:
       {
         ...
         //pvsCmd[curlist].firstmv = pendingCmd.notifymv();  FIXME set firstmv
