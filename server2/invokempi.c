    ...
    if (*mproc) {
      ...
      PROBE_INTERVAL = PROBE_INTERVAL_INITIAL;
        ...
        if (!strncmp(argv[i], "-m", 2)) {
            master_proc_offset = argv[i][2] - '0';
            if ('0' <= argv[i][3] && argv[i][3] <= '9')
              master_proc_offset = 
                 10 * master_proc_offset + argv[i][3] - '0';
...
//******************* 
//void sendPacket(int dst, unsigned int* buf, int count)
void sendPacket(int dst,          int* buf, int count)
{
    ...
    //MPI_Send(buf, count, MPI_UNSIGNED, dst, OURTAG, MPI_COMM_WORLD);
...
//******************* 
//int recvPacket(int src, unsigned int* buf)
int recvPacket(int src, int* buf)
{
    ...
    MPI_Recv(buf, MAXPACKETSIZE, MPI_UNSIGNED, src, OURTAG,
         MPI_COMM_WORLD, &status);  // FIXME?  MPI_INT?
        if (DBG_MPI) {
          int cnt; // for dbg
          char str[300];
