...
#define BKUP_TO_ALWAYS 0

#ifdef CACHE_ALIGNED_HASH

#if HASH_ENTRY_WIDTH == 4
#define hashslot(key) ((((unsigned int)(key) >> 26) * 3) >> 6)
#else  // WIDTH=2
#define hashslot(key) 0
#endif

#else
#define hashslot(key) ((unsigned int)(key) >> 31)
#endif
            ...
            unsigned int state_node )
{
  ...
  keyt_hash = hash_word2 & ~(uint64_t)0x3fU;
  keyt      =      word2 & ~(uint64_t)0x3fU;
...
clear_trans_table( void )
{
  ...
  for ( i = 0; i < ntrans_table; i++ )
    { int j;
      ...
      for(j=0; j<HASH_ENTRY_WIDTH-1; j++) {
        ptrans_table[i].always[j].word1 = 0;
        ptrans_table[i].always[j].word2 = 0;
      }
    }
