...
static int srchAsp(tree_t * restrict ptree, int val, int turn, int dep,
                    int ply, int stat) {
  int A, B, oldA, oldB;
  oldA = root_alpha;
  oldB = root_beta;

  //if (val == INITIAL_ASP_VAL ||  val == -INITIAL_ASP_VAL)
...
static unsigned int state_node = node_do_mate | node_do_null | node_do_futile
     |node_do_recap |node_do_recursion |node_do_hashcut;
static unsigned int state_node_dmy = 0;

void slave() {
  int64_t pre, post, pren, postn, sleepStarted = 0LL;
 ...
 while(1) {
   ...
   if (srchJob.onSingle()) {
     ...
     //singleCmd.clear();   // 12/11/2010 #7 WARM,FIRST only, not for ROOT
...
static void makeRootMoveList() {
         int tv = hash_probe(g_ptree, 1, NO_FUTIL_DEP * PLY_INC,
            root_turn, -score_bound, score_bound, &state_node_dmy);
 ...
 inRoot = rootExceeded = 0;
       ...
       if        (root_nmove == 1) {
          ...
          //     find two moves for next ponder
           ...
           //pren = ptree->node_searched;
           MakeMove(root_turn, mv1, 1);
           assert(!InCheck(root_turn));
           int tv = hash_probe(g_ptree, 2,
                     PUZZLE_DEP * PLY_INC + PLY_INC/2,
                     Flip(root_turn),
           ...
           if (mv2 == MOVE_NA || !is_move_valid(ptree, mv2, Flip(root_turn))) {
              ...
              int val = search(g_ptree, -score_bound, score_bound,
                         Flip(root_turn),
       ...
       forr(dep, NO_FUTIL_DEP, MAX_SRCH_DEP-1) {
         ...
         val = !USE_ASP ? search(g_ptree, -score_bound, score_bound,
                            root_turn,
         ...
         if (root_abort) {
           ...
           if (rootExceeded) {
             ...
             //singleCmd.itd++;   12/3/2011 %33 increment should not be done
         ...
         {  //**** record results in singleCmd
           ...
           //singleCmd.val = (ptree->pv[0].length - leng)%2 ? -val : val;
         ...
         if (abs(singleCmd.val) < score_max_eval &&
           ...
           if (singleCmd.pvleng == 0) {
             ...
             int tv = hash_probe(g_ptree, 1, dep * PLY_INC,
                root_turn, -score_bound, score_bound, &state_node_dmy);
             int hashmv = g_ptree->amove_hash[1];
           ...
           int tv = hash_probe(g_ptree, 2, newdep * PLY_INC,
              Flip(root_turn), -score_bound, score_bound, &state_node_dmy);
           ...
           if (hashmv != MOVE_NA && hashmv != MOVE_PASS &&
             ...
             MakeMove(Flip(root_turn), hashmv, 2);  // make sure not a selfchk
             if (!InCheck(Flip(root_turn))) {
               singleCmd.pvleng = 2;
...
extern "C" void replyFirst() {
     ...
     forr(ply, 0, singleCmd.exd-1)
      if (!(singleCmd.haveList & (1 << ply)))
       rpyent.pushSetlist(singleCmd.itd, ply, firstMvcnt[ply], firstMoves[ply]);

     rpyent.pushFirst(singleCmd.itd, singleCmd.exd - 1, -singleCmd.val,
        0 /*min(GMX_MAX_BESTSEQ-1, singleCmd.bestseqLeng)*/, singleCmd.bestseq);
...
{  // block A
 ...
 for(i=0; i<leng; i++) {
   ...
   //ptree->nsuc_check[i+2] = !chked ? 0 : i<=0 ? 1 : ptree->nsuc_check[i]+1;
   ...
   //int newdep = (itd - ply + popcnt(curPosPathChecks)) * PLY_INC ;  //+
                //(1 + popcnt(onerep)) * PLY_INC / 2;
   int tv = hash_probe(g_ptree, ply+1, newdep, turn,
                       -score_bound, score_bound, &state_node_dmy);
   if (tv == value_exact) {
       // FIXME? add hashmv to pv? (need valid/selfchk check)
     singleCmd.exd =
     singleCmd.pvleng = i+1;
...
#define VAL_MVONLY_DMY 4
 if (singleCmd.haveList & HVLST_MVONLY) {
   ...
   return;
       ...
       for(ply = singleCmd.pvleng; ply >= 1; ply--) { // 12/11/2010 #5 was <=1
           if (pren + MAX_FIRST_NODE < g_ptree->node_searched)
             break;
          ...
          //if (n>1 || ply == singleCmd.pvleng) {
           ...
           //int newdep = (itd - ply + popcnt(curPosPathChecks)) * PLY_INC;
                        //+ (1 + popcnt(singleCmd.onerepMask))*PLY_INC/2;
           int newply = ply;
           { int ply = newply+1;
             MOVE_LAST = singleCmd.pv[newply-1].v;
           }
           ...
           if (ply==1 && g_ptree->pv[ply].length < ply+1) {
             ...
             int tv = hash_probe(g_ptree, ply+1, newdep,
           ...
           if (newleng == 1 && ply < singleCmd.pvleng && singleCmd.bestseqLeng>0
             ...
             forr(i, 0, newleng-1)            // FIXME? ply+1?
               singleCmd.bestseq[i+1] = ptree->pv[ply].a[ply+i+1]; // ditto
           }
       ...
       rpyent.pushFcomp(singleCmd.itd, singleCmd.exd, singleCmd.val,
             min(GMX_MAX_BESTSEQ-1, singleCmd.bestseqLeng), singleCmd.bestseq);
       rpypkt.send();   // 11/27/2011 %9' was missing
 ...
 sprocmvsC& sprmv = splane.stream[itd].row[exd].procmvs;
     ...
     //           then set pvsretrying. send reply also
       ...
       if (!chk && curPosPathLeng>0 &&
        ...
        mv.v != ptree->killers[ply+1].no1 &&
        mv.v != ptree->killers[ply+1].no2    ) {
          int depth_reduced = 0;
         ...
         if (val>max(A,srow.alpha)) {  // 3/6/2012 %xx was val>sr.alpha
           ...
           if (!lower && g_ptree->pv[ply+1].length < ply+2) {
             ...
             if (g_ptree->pv[ply+1].length < ply+2) {
               int tv = hash_probe(g_ptree, ply+2, newdep,
                  Flip(turn), -score_bound, score_bound, &state_node_dmy);
       ...
       if (!root_abort && val<=A) { // 3/6/2012 %xx was val<=srow.alpha) {
         ...
         int tv = hash_probe(g_ptree, ply+2, newdep,
                Flip(turn), -A-1, -A, &state_node_dmy);
