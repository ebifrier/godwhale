...
static void dumpSlState()
{
    ...
    SLTOut("\nsingleCmd: valid %d issued %d havelist %04x onerep %04x\n",
           singleCmd.valid, singleCmd.issued,
           singleCmd.haveList, singleCmd.onerepMask);
    SLTOut("  itd %d pvleng %d opcode %d fromroot %d   pv:\n",
           singleCmd.itd, singleCmd.pvleng,
           singleCmd.opcode, singleCmd.fromRoot);
    ...
    forr (k, 0, 2) {
        /*SLTOut("\npvsCmd[%d]: valid %d itd %d havelist %04x onerep %04x\n",
               k,pvsCmd[k].valid, pvsCmd[k].itd,pvsCmd[k].haveList,
               pvsCmd[k].onerepMask);*/

        SLTOut("\npvsCmd[%d]: valid %d itd %d firstmv %07x onerep %04x\n",
               k,pvsCmd[k].valid, pvsCmd[k].itd,
               readable(pvsCmd[k].firstmv), pvsCmd[k].onerepMask);
        SLTOut("  exd %d pvleng %d alpha %d mvcnt %d top %d updated %d  bestmv:\n",
               pvsCmd[k].exd, pvsCmd[k].pvleng,
               pvsCmd[k].alpha, pvsCmd[k].mvcnt,
...
int displayPerf(tree_t * restrict ptree)
{
...
struct perfTimeNodeC
{
    ...
    perfTimeNodeC() {}
    perfTimeNodeC(int64_t t, int64_t n)
        : time(t), node(n) {} // FIXME? not needed?

    void clear()
    {
        time = node = 0LL; call = 0;
    }

    void add(int64_t t, int64_t n)
    {
        time += t; node += n; call++;
    }
    ...
    {
        SLTOut("perf %s: tm %12lld nd %12lld cl %8d\n",
               s, time, node, call);
    }
};
...
struct perfRecordC
{
    perfTimeNodeC perfAccumRoot, perfAccumFirst;
    perfTimeNodeC perfAccumAbort, perfAccumPuzzle;
    ...
    void clear()
    {
        memset(this, 0, sizeof(perfRecordC));
    }
