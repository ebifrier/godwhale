...
client_next_game( tree_t * restrict ptree, const char *str_addr, int iport )
{
  ...
  if ( restart_mode ) { int k;
    min_posi.turn_to_move = root_turn;
    min_posi.hand_black = ptree->posi.hand_black;
    min_posi.hand_white = ptree->posi.hand_white;
    for(k=0; k<81; k++)
      min_posi.asquare[k] = ptree->posi.asquare[k];
  } else
...
#  endif
  ...
  // with gprof, sometimes thie select() fails.
  // retrying does help here (don't know why)
 for(i=0; i<4; i++) {
  iret = select( (int)sd+1, &readfds, NULL, NULL, &tv );
  if ( iret == SOCKET_ERROR )
    {
      str_error = str_WSAError( "ch:select() with a socket connected failed:" );
      if (i>=3)
        return -2;
    }
  else break;
