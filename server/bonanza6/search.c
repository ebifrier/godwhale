...
static void CONV hist_good( tree_t * restrict ptree, move_t move,
...
#ifdef CLUSTER_PARALLEL
  ptree->pv[ply].length = ptree->pv[ply].a[ply] = 0;
#endif
...
#else
        if (Mproc && --node_next_cmdchk <= 0) { // slave (thr0) only
          node_next_cmdchk = CMDCHK_INTERVAL;
          if (detectSignalSlave()) {
            root_abort = 1;
            return 0;
          }
        }
      ...
      switch ( hash_probe( ptree, ply, depth, turn, alpha, beta,
        ...
        case value_lower:
          ...
          if ( ( state_node & node_do_hashcut )
               && beta == alpha_old + 1 ) {
#ifdef CLUSTER_PARALLEL
            ptree->pv[ply].a[ply] = MOVE_CURR;
            ptree->pv[ply].length =
              (MOVE_CURR == MOVE_NA || MOVE_CURR == MOVE_PASS) ? ply-1 : ply;
#endif
             return HASH_VALUE;
          }
          break;
...
#ifdef PREFCH_EHASH
      _mm_prefetch((const char*)&ehash_tbl[HASH_KEY & EHASH_MASK], _MM_HINT_T2);
#endif
...
static void CONV
hist_good( tree_t * restrict ptree, move_t move_good, int ply,
           int depth, int turn )
{
...
#  ifdef TLP
          unlock(&hist_lock[key]);
#  endif
        }

      assert( hist_tried[key] < HIST_MAX );
      hist_tried[key]
        = (unsigned short)( (int)hist_tried[key] + depth );
#endif
    }
...
#else
  assert( hist_goodary[key] < HIST_MAX );
  hist_goodary[key]
    = (unsigned short)( (int)hist_goodary[key] + depth );
#endif
}
