...
#if defined(TLP)
static int      cmd_affinity( char **lasts );
...
static int CONV cmd_restart( int );
...
static int CONV proce_cui( tree_t * restrict ptree )
{
  ...
  if ( ! strcmp( token, "restartx" ) )   { return cmd_restart(1); }
  if ( ! strcmp( token, "restartb" ) )   { return cmd_restart(2); }
  if ( ! strcmp( token, "restartw" ) )   { return cmd_restart(3); }
  ...
  if ( ! strcmp( token, "affinity" ) )  { return cmd_affinity( &last ); }
#endif
#if ! defined(NO_STDOUT)
...
static int CONV cmd_restart( int n )
{
  restart_mode = n;
  Out("restart mode set to %d\n", n);
  return 1;
}
...
cmd_affinity( char **lasts )
{
  ...
  l = strtol( str, &ptr, 0 );
  if ( ptr == str || l == LONG_MAX || l < 0 || l > 31 )
    {
      str_error = str_bad_cmdline;
      return -2;
    }
