...

{
    ...
    # creates a listening socket for csa server proxy.
    my $csa_listen
        = new IO::Socket::INET( LocalPort => 4090, #$status{csa_port},
                                Listen    => SOMAXCONN,
                                Proto     => 'tcp',
                                ReuseAddr => 1 )
        or die "Can't create a csa server listening socket: $!\n";
    ...
    while ( 1 ) {
        ...
        my $sckt_csa  = open_sckts( \%status, $csa_listen, $sckt_listen,
                                    \@sckt_clients, \@game_summary, $fh_log );
...
sub open_sckts ($$$$$$) {
    my ( $ref_status, $csa_listen, $sckt_listen, $ref_sckt_clients,
