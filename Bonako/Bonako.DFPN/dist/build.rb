...
$HtmlBasePath = "E:/Dropbox/NicoNico/homepage/garnet-alice.net/programs/bonako-dfpn"
...
def make_recent(appdata)
  input_path = File.join("bonako-dfpn_tmpl.html")
  output_path = File.join("bonako-dfpn.html")
...
def make_dist(appdata)
  ...
  # 作成ファイルを出力します。
  project_path = File.join(appdata.dist_path, "../Bonako.DFPN.csproj")
  appdata.build(project_path, "CLR_V4")
  setup_dist(appdata)
...
def copy_dist(appdata)
  ...
  FileUtils.copy(File.join(appdata.dist_path, "bonako-dfpn.html"), File.join($HtmlBasePath, "download"))
end
...
appdata = AppData.new("bonako-dfpn", dist_path, assemblyinfo_path, history_path)
