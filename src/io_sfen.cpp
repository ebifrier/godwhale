...
Move sfenToMove(Position const & position, std::string const & sfen)
{
    ...
    else
    {
        ...
        return Move::create(from, to, abs(piece), capture, promote);
    }
}
#pragma endregion
 ...
 */
static std::string rankToSfen(Position const & position, int rank)
{
    std::string result;
    int nspaces = 0;

    for (int file = nfile; file >= 1; --file) {
        piece_t piece = position.get(file, rank);
        if (piece == 0) {
            // 駒がない場合
            nspaces += 1;
        }
 ...
 */
static std::string position0ToSfen(Position const & position)
{
 ...
 */
static std::string hand0ToSfen(Position const & position, int turn)
{
    std::string result;
 ...
 */
static std::string handToSfen(Position const & position)
{
    std::string result;
 ...
 */
static void parseBoard0(Position & position, std::string const & sfen)
{
    int rank = 1;
    int file = 9;
    int promoted = false;
 ...
 * @brief 持ち駒を読み込みます。
 */
static void parseHand(Position & position, std::string const & sfen)
{
    if (sfen[0] == '-') {
        // 何もする必要がありません。
        return;
